namespace NetworkTools.Systems.Tools {
    ...
    /// <summary>
    ///     Shared snapping capability for all tool systems. Lives on the base tool (which is
    ///     already a <see cref="Unity.Entities.SystemBase" />) rather than as its own registered
    ///     system, because snapping is invoked synchronously from the tool's main-thread update
    ///     and has no independent tick. Consumed by both control-point placement and handle
    ///     dragging via <see cref="TrySnapWorld" />.
    /// </summary>
    public abstract partial class NT_BaseToolSystem {
        // Search systems backing the spatial snap job.
        ...
        /// <summary>
        ///     Snap lines produced by the most recent <see cref="TrySnapWorld" /> call.
        ///     Currently unrendered (see plan); exposed so a future overlay pass can consume them.
        /// </summary>
        protected NativeList<SnapLine> SnapLines => m_SnapLines;

        /// <summary>Allocates snap resources and fetches search systems. Called from OnCreate().</summary>
        ...
        /// <summary>Disposes snap resources. Called from OnDestroy().</summary>
        ...
        /// <summary>
        ///     Net prefab used by the spatial snap job to derive width, snap distance and
        ///     height ranges. Override in tools that place a net. Returns
        ///     <see cref="Entity.Null" /> by default, which disables world snapping.
        /// </summary>
        protected virtual Entity GetSnapPrefab() {
            return Entity.Null;
        }

        /// <summary>Elevation offset fed to the spatial snap job's height checks. Override as needed.</summary>
        protected virtual float GetSnapElevation() {
            return 0f;
        }

        /// <summary>
        ///     Runs the shared spatial snap job against <paramref name="rawHit" />. Schedules the
        ///     job on <paramref name="inputDeps" /> combined with the search-tree dependencies,
        ///     registers the search-tree readers, and <c>Complete()</c>s so the caller can read
        ///     the result on the main thread the same frame. Returns the combined handle so the
        ///     caller can fold it into the tool's OnUpdate output.
        /// </summary>
        /// <param name="rawHit">Raw hit (cursor or constrained projection) to snap.</param>
        /// <param name="flags">Active snap options to consider.</param>
        /// <param name="prefab">Net prefab being placed (see <see cref="GetSnapPrefab" />).</param>
        /// <param name="elevation">Elevation offset for height checks.</param>
        /// <param name="inputDeps">The frame's job dependency to schedule against.</param>
        /// <param name="snapped">The snapped control point (equals <paramref name="rawHit" /> if nothing won).</param>
        /// <param name="won">True when a snap candidate beat the raw hit.</param>
        /// <returns>The combined job handle (already completed).</returns>
        protected JobHandle TrySnapWorld(
            ControlPoint     rawHit,
            SnapOption       flags,
            Entity           prefab,
            float            elevation,
            JobHandle        inputDeps,
            out ControlPoint snapped,
            out bool         won) {
            snapped = rawHit;
            won     = false;

            if (prefab == Entity.Null || flags == SnapOption.None) {
                return inputDeps;
            }
            ...
            var waterData = m_WaterSystem.GetSurfaceData(out var waterDep);

            var snapJob = new SnapPlacementJob {
                m_ControlPoint = rawHit,
                m_SnapFlags    = flags,
                m_Prefab       = prefab,
            ...
            var s = m_SnappedControlPoint.value;
            won = s.m_OriginalEntity != Entity.Null || s.m_SnapPriority.x > 0f || s.m_SnapPriority.y > 0f;
            if (won) {
                snapped = s;
            }
    ...
    [BurstCompile]
        ...
        internal struct SnapPlacementJob : IJob {
            ...
            [ReadOnly] public ComponentLookup<Node>               m_NodeData;
            ...
            public void Execute() {
                ...
                var              netData          = m_PrefabNetData[m_Prefab];
            ...
            /// </summary>
            private void HandleExistingGeometry(ref ControlPoint bestSnapPosition, NetData netData, RoadData roadData,
                                                NetGeometryData  netGeometryData,  PlaceableNetData placeableNetData) {
                ...
                var bounds = new Bounds3 {
                    xz = new Bounds2(m_ControlPoint.m_HitPosition.xz - searchRadius,
                                     m_ControlPoint.m_HitPosition.xz + searchRadius),
                ...
                if ((m_SnapFlags & SnapOption.ExistingGeometry) != SnapOption.None &&
                    m_PrefabRefData.HasComponent(m_ControlPoint.m_OriginalEntity)) {
            ...
            /// </summary>
            private void HandleExistingObjects(ref ControlPoint bestSnapPosition, NetData          netData,
                                               NetGeometryData  netGeometryData,  PlaceableNetData placeableNetData) {
                var snapOffset = netGeometryData.m_DefaultWidth * 0.5f + placeableNetData.m_SnapDistance;
                var bounds = new Bounds3(m_ControlPoint.m_HitPosition - snapOffset,
                ...
                m_ObjectSearchTree.Iterate(ref objectIterator);
            ...
            private void HandleZoneGrid(ref ControlPoint bestSnapPosition, NetGeometryData netGeometryData) {
                var cellWidth = ZoneUtils.GetCellWidth(netGeometryData.m_DefaultWidth);
                // Search radius: (cellWidth + 1) cells * 4m per half-cell * sqrt(2) for diagonal
                var searchRadius = (cellWidth + 1) * 4f * 1.4142135f;
                // Even-width nets need a half-cell offset to center on cell boundaries
                var cellOffset = math.select(0f, 4f, (cellWidth & 1) == 0);

                var zoneIterator = new ZoneIterator {
                    m_Bounds = new Bounds2(m_ControlPoint.m_HitPosition.xz - searchRadius,
                                           m_ControlPoint.m_HitPosition.xz + searchRadius),
                ...
                m_ZoneSearchTree.Iterate(ref zoneIterator);
                ...
                if (zoneIterator.m_BestDistance < searchRadius) {
                    ...
                    if (!m_EdgeData.HasComponent(m_ControlPoint.m_OriginalEntity) &&
                        !m_NodeData.HasComponent(m_ControlPoint.m_OriginalEntity)) {
                        candidate.m_OriginalEntity = Entity.Null;
                    }

                    candidate.m_Direction = zoneIterator.m_BestDirection;
                    candidate.m_Position.xz = zoneIterator.m_BestPosition.xz             +
                                              zoneIterator.m_BestDirection * snapForward + right * snapRight;
            ...
            ///         <item>
            ///             <b>Edge snap</b> — snaps to the closest point on a curve (via <see cref="HandleCurve" />),
            ///             including segment-area snapping for nets with buildable net areas
            ///         </item>
            ///         <item>
            ///             <b>Guide lines</b> — emits directional guide lines extending from
            ///             edge endpoints, with perpendicular lines at dead-end nodes
            ///         </item>
            ///     </list>
            ...
            private struct NetIterator : INativeQuadTreeIterator<Entity, QuadTreeBoundsXZ>,
                ...
                public void Iterate(QuadTreeBoundsXZ bounds, Entity entity) {
                    ...
                    if (entity                                      == m_ControlPoint.m_OriginalEntity &&
                        (m_SnapFlags & SnapOption.ExistingGeometry) != SnapOption.None) {
                        return;
                    }

                    // Try geometry snap first; if it succeeds, skip guide lines for this entity
                    if (MathUtils.Intersect(bounds.m_Bounds, m_Bounds)                 &&
                        (m_SnapFlags & SnapOption.ExistingGeometry) != SnapOption.None && HandleGeometry(entity)) {
                        return;
                    }
                ...
                public bool HandleGeometry(Entity entity) {
                    ...
                    if (m_CurveData.HasComponent(entity)) {
                        ...
                        if (MathUtils.Distance(curve.m_Bezier.xz,
                                               m_ControlPoint.m_HitPosition.xz,
                                               out controlPoint.m_CurvePosition) >= snapRadius) {
                            return false;
                        }
                ...
                public bool HandleGeometry(ControlPoint controlPoint, float snapHeight, PrefabRef prefabRef,
                    ...
                    float baseHeight;
                    if (m_Elevation < 0f) {
                        baseHeight = TerrainUtils.SampleHeight(ref m_TerrainHeightData, controlPoint.m_HitPosition) +
                                     m_Elevation;
                    } else {
                        baseHeight =
                    ...
                    if (m_NodeData.HasComponent(controlPoint.m_OriginalEntity)) {
                            ...
                            if (connectedEdges.Length != 0) {
                                for (var i = 0; i < connectedEdges.Length; i++) {
                                    ...
                                    if (edge.m_Start == controlPoint.m_OriginalEntity ||
                                        edge.m_End   == controlPoint.m_OriginalEntity) {
                        ...
                        candidate.m_SnapPriority =
                            ToolUtils.CalculateSnapPriority(snapPrio,
                                                            1f,
                                                            1f,
                ...
                /// </summary>
                private void HandleCurve(ControlPoint controlPoint, Entity curveEntity, bool allowEdgeSnap,
                                         ref bool     snapAdded) {
                    ...
                    var             prefabRef = m_PrefabRefData[curveEntity];
                    ...
                    if (!allowEdgeSnap && curvePos > 0f && curvePos < 1f) {
                        if (curvePos >= 0.5f) {
                            if (math.distance(curve.m_Bezier.d.xz, controlPoint.m_HitPosition.xz) <
                                halfWidth + m_SnapOffset) {
                                curvePos                  = 1f;
                            ...
                            if (math.distance(curve.m_Bezier.a.xz, controlPoint.m_HitPosition.xz) <
                                halfWidth + m_SnapOffset) {
                                curvePos                  = 0f;
                    ...
                    ToolUtils.AddSnapLine(ref m_BestSnapPosition,
                                          m_SnapLines,
                                          new SnapLine(candidate, curve.m_Bezier, snapLineFlags, 1f));
                    snapAdded = true;
                }
                ...
                /// </summary>
                private bool SnapSegmentAreas(ControlPoint controlPoint, NetCompositionData compositionData,
                                              DynamicBuffer<NetCompositionArea> areas, Segment segment,
                                              ref bool snapAdded) {
                    var hasArea = false;
                    for (var i = 0; i < areas.Length; i++) {
                        ...
                        var areaHalfWidth = area.m_Width * 0.51f;
                        if (m_NetGeometryData.m_DefaultWidth * 0.5f >= areaHalfWidth) {
                            continue;
                        }

                        hasArea = true;
                        var areaCurve = MathUtils.Lerp(segment.m_Left,
                                                       segment.m_Right,
                                                       area.m_Position.x / compositionData.m_Width + 0.5f);
                        float areaT;
                        ...
                        candidate.m_SnapPriority =
                            ToolUtils.CalculateSnapPriority(1f,
                                                            1f,
                                                            1f,
                                                            controlPoint.m_HitPosition,
                                                            candidate.m_Position,
                        ...
                        ToolUtils.AddSnapLine(ref m_BestSnapPosition,
                                              m_SnapLines,
                                              new SnapLine(candidate, areaCurve, snapLineFlags, 1f));
                        snapAdded = true;
                    }
                ...
                public void HandleGuideLines(Entity entity) {
                    ...
                    var hasStart = MathUtils.TryNormalize(ref startDir);
                    var hasEnd   = MathUtils.TryNormalize(ref endDir);

                    // Check if start/end nodes are dead-ends (single connected edge)
                    var startDeadEnd = hasStart;
                    if (hasStart) {
            ...
            ///     <list type="bullet">
            ///         <item>
            ///             <b>Object side</b> — snaps to the four lot edges of rectangular buildings.
            ///             The lot is expanded by <see cref="m_ObjectSnapOffset" /> to catch near-misses.
            ///         </item>
            ///         <item>
            ///             <b>Owner node</b> — if the object owns a net node (e.g. a bus stop),
            ///             snaps to that node's position (delegated to <see cref="SnapToNode" />).
            ///         </item>
            ///     </list>
            /// </summary>
            private struct ObjectIterator : INativeQuadTreeIterator<Entity, QuadTreeBoundsXZ>,
                                            IUnsafeQuadTreeIterator<Entity, QuadTreeBoundsXZ> {
                public Bounds3                             m_Bounds;
                public SnapOption                          m_SnapFlags;
                ...
                public void Iterate(QuadTreeBoundsXZ bounds, Entity entity) {
                    ...
                    if ((m_SnapFlags & SnapOption.ExistingGeometry) != SnapOption.None &&
                        m_OwnerData.HasComponent(entity)) {
                ...
                private void SnapToNode(Entity entity) {
                    ...
                    var dist       = math.distance(node.m_Position.xz, m_ControlPoint.m_HitPosition.xz);
                    var snapRadius = m_NetGeometryData.m_DefaultWidth * 0.5f;
                ...
                private void SnapObjectSide(Entity entity) {
                    ...
                    var quad = ObjectUtils.CalculateBaseCorners(transform.m_Position,
                ...
                private void CheckLine(Line3.Segment line) {
                    ...
                    if (MathUtils.Distance(line.xz, m_ControlPoint.m_HitPosition.xz, out t) < m_MaxDistance) {
                        ...
                        if (m_CurveData.HasComponent(m_ControlPoint.m_OriginalEntity)) {
                            MathUtils.Distance(m_CurveData[m_ControlPoint.m_OriginalEntity].m_Bezier.xz,
                                               candidate.m_Position.xz,
                                               out candidate.m_CurvePosition);
            ...
            private struct ZoneIterator : INativeQuadTreeIterator<Entity, Bounds2>,
                ...
                public void Iterate(Bounds2 bounds, Entity entity) {
                    ...
                    for (var y = 0; y < block.m_Size.y; y++) {
                        for (var x = 0; x < block.m_Size.x; x++) {
