namespace NetworkTools.Systems.UI {
    ...
    public partial class NT_UISystem : CommonUISystemBase {
        ...
        protected override void OnCreate() {
            ...
            m_SelectedTargetsBinding =
                CreateBinding("SELECTED_TARGETS", (int)TargetOption.All, HandleUpdateSelectedTargets);
            ...
            m_ApplyTransformationAction =
                NetworkToolsMod.Instance.Settings.GetAction(NT_Settings.ApplyTransformationStr);
        ...
        private NT_BaseToolSystem GetActiveToolDefinition() {
            return m_ToolSystem.activeTool as NT_BaseToolSystem;
        }
        ...
        private void RegisterParameterBinding(ParameterBase param) {
            if (param is FloatParameter fp) {
                ...
                var b = CreateBinding(bp.Key, bp.Value, v => bp.Value = v);
                bp.OnChanged += _ => b.Value = bp.Value;
            } else if (param is IEnumParameter ep) {
                var b = CreateBinding(ep.Key, ep.IntValue, v => ep.IntValue = v);
                ...
                np.OnChanged += _ =>
                {
                    b.Value = np.Prefab != null
                                  ? new NetPrefabData(np.NetLanePrefabEntity != Entity.Null
                                                          ? np.NetLanePrefabEntity
                                                          : np.NetPrefabEntity,
